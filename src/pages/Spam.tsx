...
const Spam = () => {
  ...
  let spamEmails = spamData.emails as MessageType[];

  if (filter === "Read") {
    spamEmails = spamEmails.filter((email) => email.read);
  } else if (filter === "Unread") {
    spamEmails = spamEmails.filter((email) => !email.read);
  } else if (filter === "Today") {
    const todayLabel = "Now";
    spamEmails = spamEmails.filter((email) => email.time === todayLabel);
  }

  if (searchTerm) {
    const lowerSearchTerm = searchTerm.toLowerCase();
    spamEmails = spamEmails.filter(
      (email) =>
          ...
          <React.Fragment key={email.id.toString()}>
            <ListItem alignItems="center" disablePadding>
              ...
              >
                <ListItemAvatar>
                  <Avatar
                    src={`https://i.pravatar.cc/150?u=spam-${email.id}`}
